...
Filewalker.prototype._buildIgnoreFolderRegex = function(){
    if(this.ignoredFolders.length == 0) {
        return;
    }
    var regex = '/';
    this.ignoredFolders.forEach(function(folder){
        regex += '(' + folder + ')|'; 
    });
    regex.substring(-1) + '/';
    console.log(regex);
    this.ignoredFoldersRegex = new RegExp(regex);
};
...
Filewalker.prototype._stat = function(p) {
  ...
  fs[lstat](p, function(err, s) {
    if(err) {
      ...
      self.total += 1;
      if((s.isDirectory())&&(!self.ignoredFoldersRegex || !self.ignoredFoldersRegex.test(p))) {
        self.enqueue(self._emitDir, [self._path(p), s, p]);
      } else {
        if((!self.ignoreFiles)&&(!self.matchRegExp || self.matchRegExp.test(p))) {
